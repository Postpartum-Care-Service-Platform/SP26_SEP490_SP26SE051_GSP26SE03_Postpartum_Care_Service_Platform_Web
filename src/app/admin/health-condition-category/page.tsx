...
import { Download, Upload, FileIcon, HeartPulse } from 'lucide-react';
...
import { HealthConditionCategoryModal } from './components/HealthConditionCategoryModal';
import { ImportHealthConditionCategoryModal } from './components/ImportHealthConditionCategoryModal';
import { ConfirmModal } from '@/components/ui/modal/ConfirmModal';
import styles from '../package-type/package-type.module.css'; // Reusing styles if possible or creating new
...
const getTranslatedCategoryName = (name: string) => {
  ...
  switch (normalized) {
    case 'preference':
      return 'Sở thích cá nhân';
    case 'baby status':
      return 'Tình trạng em bé';
    case 'delivery':
      return 'Hình thức sinh';
    case 'chronic':
...
const sortItems = (items: HealthConditionCategory[], key: SortKey) => {
  const arr = [...items];
  switch (key) {
    case 'id-asc':    return arr.sort((a, b) => a.id - b.id);
    case 'id-desc':   return arr.sort((a, b) => b.id - a.id);
    case 'name-asc':  return arr.sort((a, b) => getTranslatedCategoryName(a.name).localeCompare(getTranslatedCategoryName(b.name)));
    case 'name-desc': return arr.sort((a, b) => getTranslatedCategoryName(b.name).localeCompare(getTranslatedCategoryName(a.name)));
    default: return arr;
  }
};

export default function AdminHealthConditionCategoryPage() {
  const [items, setItems]         = useState<HealthConditionCategory[]>([]);
  ...
  const [editingItem, setEditingItem] = useState<HealthConditionCategory | null>(null);
  const [updatingStatusIds, setUpdatingStatusIds] = useState<Set<number>>(new Set());

  const [isImportModalOpen, setIsImportModalOpen] = useState(false);
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<HealthConditionCategory | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const data = await healthConditionService.getAllCategories();
      setItems(data);
    } catch (err: unknown) {
      setError(getErrorMessage(err, 'Không thể tải danh sách danh mục tình trạng sức khỏe'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleOpenEdit   = (item: HealthConditionCategory) => { setEditingItem(item); setIsModalOpen(true); };
  const handleModalClose = (open: boolean) => { setIsModalOpen(open); if (!open) setEditingItem(null); };

  const handleDeleteTrigger = (item: HealthConditionCategory) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await healthConditionService.deleteCategory(itemToDelete.id);
      await fetchData();
      toast({ title: 'Xóa danh mục thành công', variant: 'success' });
    } catch (err) {
      toast({ title: getErrorMessage(err, 'Xóa danh mục thất bại'), variant: 'error' });
    } finally {
      setDeletingId(null);
  ...
  const handleRestore = async (item: HealthConditionCategory) => {
    try {
      setDeletingId(item.id);
      await healthConditionService.restoreCategory(item.id);
      await fetchData();
      toast({ title: 'Khôi phục danh mục thành công', variant: 'success' });
    } catch (err) {
      toast({ title: getErrorMessage(err, 'Khôi phục danh mục thất bại'), variant: 'error' });
    } finally {
      setDeletingId(null);
    }
  };

  const handleToggleStatus = async (item: HealthConditionCategory, newStatus: boolean) => {
    const isCurrentlyActive = !item.isDeleted;
    if (isCurrentlyActive === newStatus || updatingStatusIds.has(item.id)) return;
    try {
      ...
      if (newStatus) {
        ...
        await healthConditionService.deleteCategory(item.id);
        toast({ title: 'Tạm dừng danh mục thành công', variant: 'success' });
      }
      await fetchData();
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await healthConditionService.exportCategories();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `Danh_muc_suc_khoe_${new Date().getTime()}.xlsx`);
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const blob = await healthConditionService.downloadTemplateCategories();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Mau_nhap_danh_muc_suc_khoe.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    ...
    <div className={styles.controls}>
      <div className={styles.controlsLeft}>
        <div className={styles.searchWrapper}>
          ...
          <input type="text" placeholder="Tìm kiếm danh mục..." className={styles.searchInput} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
      ...
      <div className={styles.controlsRight}>
        ...
        <Button variant="primary" size="sm" className={styles.createButton} onClick={handleOpenCreate}>
          <PlusIcon className={styles.plusIcon} /> Danh mục mới
        </Button>
      </div>
    </div>
  );
      ...
      <AdminPageLayout controlPanel={controlPanel} pagination={pagination}>
        <div className={styles.tableWrapper}>
          <table className={styles.table}>
            ...
            <tbody>
              {error ? (
                <tr><td colSpan={4} className={styles.placeholder}>{error}</td></tr>
              ) : paginatedItems.length === 0 ? (
                <tr><td colSpan={4} className={styles.emptyState}>
                  {searchQuery ? 'Không tìm thấy kết quả phù hợp.' : 'Chưa có danh mục nào.'}
                </td></tr>
              ) : (
                paginatedItems.map((item, index) => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td className={styles.nameCell}>{getTranslatedCategoryName(item.name)}</td>
                      <td>
                        <div className={styles.tooltipWrapper}>
                          <DropdownMenu modal={false}>
                            <DropdownMenuTrigger asChild>
                              <button type="button" className={`${styles.plainTrigger} ${isActive ? styles.statusActive : styles.statusInactive}`} disabled={updatingStatusIds.has(item.id)}>
                          ...
                          <span className={styles.tooltip}>{isActive ? 'Đang hoạt động' : 'Tạm dừng'}</span>
                        </div>
                      </td>
                      <td>
                        <div className={styles.actions}>
                          ...
                          {isActive ? (
                            ...
                            <div className={styles.tooltipWrapper}>
                              <Button variant="outline" size="sm" className={styles.restoreButton} onClick={() => handleRestore(item)} disabled={deletingId === item.id}><RestoreIcon /></Button>
        ...
        <HealthConditionCategoryModal open={isModalOpen} onOpenChange={handleModalClose} item={editingItem} onSuccess={fetchData} />
        <ConfirmModal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} onConfirm={handleConfirmDelete} title="Xác nhận xóa danh mục" message={`Bạn có chắc chắn muốn xóa danh mục "${itemToDelete ? getTranslatedCategoryName(itemToDelete.name) : ''}"? Hành động này không thể hoàn tác.`} confirmLabel="Xóa ngay" cancelLabel="Suy nghĩ lại" variant="danger" />
        <ImportHealthConditionCategoryModal open={isImportModalOpen} onOpenChange={setIsImportModalOpen} onSuccess={fetchData} />
      </AdminPageLayout>
    </div>
  );
}
