...
import styles from './import-amenity-service-modal.module.css';

interface ImportAmenityServiceModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

export const ImportAmenityServiceModal: React.FC<ImportAmenityServiceModalProps> = ({
  open,
  onOpenChange,
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsUploading(true);
      await amenityServiceService.importAmenityServices(file);
    ...
    <div className={styles.modalOverlay} onClick={() => !isUploading && onOpenChange(false)}>
      ...
      >
        <div className={styles.modalHeader}>
          <h2 className={styles.modalTitle}>Nhập dịch vụ tiện ích từ Excel</h2>
