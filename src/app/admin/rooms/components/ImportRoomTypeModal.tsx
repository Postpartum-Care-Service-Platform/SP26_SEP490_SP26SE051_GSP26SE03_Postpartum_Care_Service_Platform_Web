...
import styles from './import-room-type-modal.module.css';

interface ImportRoomTypeModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

export const ImportRoomTypeModal: React.FC<ImportRoomTypeModalProps> = ({
  open,
  onOpenChange,
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsUploading(true);
      await roomTypeService.importRoomTypes(file);
    ...
    <div className={styles.modalOverlay} onClick={() => !isUploading && onOpenChange(false)}>
      ...
      >
        <div className={styles.modalHeader}>
          <h2 className={styles.modalTitle}>Nhập loại phòng từ Excel</h2>
