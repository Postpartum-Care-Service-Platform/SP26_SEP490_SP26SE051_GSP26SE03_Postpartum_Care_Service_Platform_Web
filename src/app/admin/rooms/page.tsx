          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '4px', border: '1px solid #f1f5f9', overflow: 'hidden' }}>
            ...
            {[...Array(pageSize)].map((_, i) => (
              <div key={i} style={{ height: '64px', borderBottom: i === pageSize - 1 ? 'none' : '1px solid #f8fafc', display: 'flex', alignItems: 'center', padding: '0 24px', gap: '24px' }}>
                ...
                <div style={{ flex: 1 }}><SkeletonBone width="50%" height={16} /></div>
                <div style={{ width: '150px' }}><SkeletonBone width="80%" height={16} /></div>
                <div style={{ width: '80px' }}><SkeletonBone width="60%" height={16} /></div>
                <div style={{ width: '120px' }}><SkeletonBone width="70%" height={16} /></div>
    ...
    <div className={styles.controls}>
      <div className={styles.controlsLeft}>
        <div className={styles.searchWrapper}>
          ...
          <input type="text" placeholder="Tìm kiếm loại phòng..." className={styles.searchInput} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-0 bg-white">
      <AdminPageLayout controlPanel={controlPanel} pagination={pagination}>
                ...
                paginatedItems.map((room, index) => {
                  ...
                  return (
                    <tr key={room.id}>
                      ...
                      <td className={styles.descCell}>{room.description || '-'}</td>
                      <td>{room.capacity ?? '-'}</td>
                      ...
                      <td>
                        <div className={styles.tooltipWrapper}>
                          <DropdownMenu modal={false}>
                            <DropdownMenuTrigger asChild>
                              <button type="button" className={`${styles.plainTrigger} ${room.isActive ? styles.statusActive : styles.statusInactive}`} disabled={updatingStatusIds.has(room.id)}>
                            ...
                            <DropdownMenuContent className={styles.statusMenu} align="start" sideOffset={4}>
                              <DropdownMenuItem className={styles.statusMenuItem} onClick={() => handleToggleStatus(room, true)}>Hoạt động</DropdownMenuItem>
                              <DropdownMenuItem className={styles.statusMenuItem} onClick={() => handleToggleStatus(room, false)}>Tạm dừng</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                          <span className={styles.tooltip}>{room.isActive ? 'Đang hoạt động' : 'Tạm dừng'}</span>
                        </div>
                      </td>
                      <td>
                        <div className={styles.actions}>
                          <div className={styles.tooltipWrapper}>
                            <Button variant="outline" size="sm" className={styles.editButton} onClick={() => handleOpenEdit(room)}><EditIcon /></Button>
                            <span className={styles.tooltip}>Chỉnh sửa</span>
                          </div>
                          {room.isActive ? (
                            <div className={styles.tooltipWrapper}>
                              <Button variant="outline" size="sm" className={styles.deleteButton} onClick={() => handleDeleteTrigger(room)} disabled={deletingId === room.id}><TrashIcon /></Button>
                              <span className={styles.tooltip}>Xóa</span>
                            </div>
                          ) : (
                            <div className={styles.tooltipWrapper}>
                              <Button variant="outline" size="sm" className={styles.restoreButton} onClick={() => handleRestore(room)} disabled={deletingId === room.id}><RestoreIcon /></Button>
        ...
        <RoomTypeModal open={isModalOpen} onOpenChange={handleModalClose} room={editingRoom} onSuccess={fetchData} />
        <ConfirmModal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} onConfirm={handleConfirmDelete} title="Xác nhận xóa loại phòng" message={`Bạn có chắc chắn muốn xóa loại phòng "${itemToDelete?.name}"? Hành động này không thể hoàn tác.`} confirmLabel="Xóa ngay" cancelLabel="Suy nghĩ lại" variant="danger" />
        <ImportRoomTypeModal open={isImportModalOpen} onOpenChange={setIsImportModalOpen} onSuccess={fetchData} />
      </AdminPageLayout>
    </div>
  );
}
