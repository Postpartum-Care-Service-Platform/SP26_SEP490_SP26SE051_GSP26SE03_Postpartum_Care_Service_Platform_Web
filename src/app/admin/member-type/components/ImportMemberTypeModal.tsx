...
import styles from './import-member-type-modal.module.css';

interface ImportMemberTypeModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

export const ImportMemberTypeModal: React.FC<ImportMemberTypeModalProps> = ({
  open,
  onOpenChange,
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsUploading(true);
      await memberTypeService.importMemberTypes(file);
    ...
    <div className={styles.modalOverlay} onClick={() => !isUploading && onOpenChange(false)}>
      ...
      >
        <div className={styles.modalHeader}>
          <h2 className={styles.modalTitle}>Nhập loại thành viên từ Excel</h2>
