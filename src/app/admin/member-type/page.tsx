...
const mapRoleName = (name: string | null | undefined) =>
  name ? (ROLE_LABELS[name.toLowerCase()] ?? name) : '—';
...
const sortItems = (items: MemberType[], key: SortKey) => {
  const arr = [...items];
  switch (key) {
...
export default function AdminMemberTypePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(getErrorMessage(err, 'Không thể tải danh sách loại thành viên'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleToggleStatus = async (item: MemberType, newStatus: boolean) => {
    ...
    try {
      ...
      if (newStatus) {
        ...
        toast({ title: 'Tạm dừng loại thành viên thành công', variant: 'success' });
      }
      await fetchData();
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await memberTypeService.exportMemberTypes();
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const blob = await memberTypeService.downloadTemplateMemberTypes();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Mau_nhap_loai_thanh_vien.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
