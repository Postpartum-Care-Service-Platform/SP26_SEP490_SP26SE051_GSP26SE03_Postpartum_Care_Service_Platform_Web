...
const sortItems = (items: FeedbackType[], key: SortKey) => {
  const arr = [...items];
  switch (key) {
...
export default function AdminFeedbackTypesPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(getErrorMessage(err, 'Không thể tải danh sách loại phản hồi'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleOpenEdit   = (item: FeedbackType) => { setEditingItem(item); setIsModalOpen(true); };
  ...
  const handleRestore = async (item: FeedbackType) => {
    try {
      ...
      await feedbackTypeService.restoreFeedbackType(item.id);
      await fetchData();
      toast({ title: 'Khôi phục loại phản hồi thành công', variant: 'success' });
    } catch (err) {
      toast({ title: getErrorMessage(err, 'Khôi phục loại phản hồi thất bại'), variant: 'error' });
    } finally {
      setDeletingId(null);
    }
  };

  const handleToggleStatus = async (item: FeedbackType, newStatus: boolean) => {
    ...
    try {
      ...
      if (newStatus) {
        ...
        toast({ title: 'Tạm dừng loại phản hồi thành công', variant: 'success' });
      }
      await fetchData();
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await feedbackTypeService.exportFeedbackTypes();
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const blob = await feedbackTypeService.downloadTemplateFeedbackTypes();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Mau_nhap_loai_phan_hoi.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    ...
    <div className={styles.controls}>
      <div className={styles.controlsLeft}>
        <div className={styles.searchWrapper}>
          ...
          <input type="text" placeholder="Tìm kiếm loại phản hồi..." className={styles.searchInput} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
      ...
      <div className={styles.controlsRight}>
        ...
        <Button variant="primary" size="sm" className={styles.createButton} onClick={handleOpenCreate}>
          <PlusIcon className={PlusIcon} /> Loại mới
        </Button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-0 bg-white">
      <AdminPageLayout controlPanel={controlPanel} pagination={pagination}>
            ...
            <tbody>
              {error ? (
                ...
                <tr><td colSpan={4} className={styles.emptyState}>
                  {searchQuery ? 'Không tìm thấy kết quả phù hợp.' : 'Chưa có loại phản hồi nào.'}
                </td></tr>
              ) : (
        ...
        <FeedbackTypeModal open={isModalOpen} onOpenChange={handleModalClose} onSuccess={fetchData} itemToEdit={editingItem} />
        <ConfirmModal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} onConfirm={handleConfirmDelete} title="Xác nhận xóa loại phản hồi" message={`Bạn có chắc chắn muốn xóa loại phản hồi "${itemToDelete?.name}"? Hành động này không thể hoàn tác.`} confirmLabel="Xóa ngay" cancelLabel="Suy nghĩ lại" variant="danger" />
        <ImportFeedbackTypeModal open={isImportModalOpen} onOpenChange={setIsImportModalOpen} onSuccess={fetchData} />
      </AdminPageLayout>
    </div>
  );
}
