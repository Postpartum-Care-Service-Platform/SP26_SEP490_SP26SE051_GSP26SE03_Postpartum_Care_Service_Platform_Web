...
import styles from './import-food-type-modal.module.css';

interface ImportFoodTypeModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

export const ImportFoodTypeModal: React.FC<ImportFoodTypeModalProps> = ({
  open,
  onOpenChange,
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsUploading(true);
      await foodTypeService.importFoodTypes(file);
    ...
    <div className={styles.modalOverlay} onClick={() => !isUploading && onOpenChange(false)}>
      ...
      >
        <div className={styles.modalHeader}>
          <h2 className={styles.modalTitle}>Nhập loại món ăn từ Excel</h2>
