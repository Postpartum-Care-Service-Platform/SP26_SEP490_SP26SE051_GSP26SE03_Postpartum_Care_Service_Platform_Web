...
const sortItems = (items: FoodType[], key: SortKey) => {
  const arr = [...items];
  switch (key) {
...
export default function AdminFoodTypePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const data = await foodTypeService.getAllFoodTypes();
      setItems(data);
    } catch (err: unknown) {
      setError(getErrorMessage(err, 'Không thể tải danh sách loại thực phẩm'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleRestore = async (item: FoodType) => {
    try {
      setDeletingId(item.id);
      await foodTypeService.restoreFoodType(item.id);
      await fetchData();
      toast({ title: 'Khôi phục loại thực phẩm thành công', variant: 'success' });
    } catch (err) {
      toast({ title: getErrorMessage(err, 'Khôi phục loại thực phẩm thất bại'), variant: 'error' });
    } finally {
      setDeletingId(null);
    }
  };

  const handleToggleStatus = async (item: FoodType, newStatus: boolean) => {
    ...
    try {
      ...
      if (newStatus) {
        ...
        toast({ title: 'Tạm dừng loại thực phẩm thành công', variant: 'success' });
      }
      await fetchData();
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await foodTypeService.exportFoodTypes();
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const blob = await foodTypeService.downloadTemplateFoodTypes();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Mau_nhap_loai_thuc_pham.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    ...
    <div className={styles.controls}>
      <div className={styles.controlsLeft}>
        <div className={styles.searchWrapper}>
          ...
          <input type="text" placeholder="Tìm kiếm loại thực phẩm..." className={styles.searchInput} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-0 bg-white">
      <AdminPageLayout controlPanel={controlPanel} pagination={pagination}>
            ...
            <tbody>
              {error ? (
                ...
                <tr><td colSpan={4} className={styles.emptyState}>
                  {searchQuery ? 'Không tìm thấy kết quả phù hợp.' : 'Chưa có loại thực phẩm nào.'}
                </td></tr>
              ) : (
        ...
        <FoodTypeModal open={isModalOpen} onOpenChange={handleModalClose} item={editingItem} onSuccess={fetchData} />
        <ConfirmModal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} onConfirm={handleConfirmDelete} title="Xác nhận xóa loại thực phẩm" message={`Bạn có chắc chắn muốn xóa loại thực phẩm "${itemToDelete?.name}"? Hành động này không thể hoàn tác.`} confirmLabel="Xóa ngay" cancelLabel="Suy nghĩ lại" variant="danger" />
        <ImportFoodTypeModal open={isImportModalOpen} onOpenChange={setIsImportModalOpen} onSuccess={fetchData} />
      </AdminPageLayout>
    </div>
  );
}
