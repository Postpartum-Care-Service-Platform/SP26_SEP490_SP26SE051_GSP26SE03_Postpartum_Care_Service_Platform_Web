...
type Props = {
  ...
  onEdit?: (appointmentType: AppointmentTypeDetail) => void;
  onDelete?: (appointmentType: AppointmentTypeDetail) => void;
...
export function AppointmentTypeTable({ appointmentTypes, onEdit, onDelete, deletingId, pagination }: Props) {
  return (
        ...
        <tbody>
          {appointmentTypes.length === 0 ? (
            ...
            appointmentTypes.map((appointmentType) => (
              <tr key={appointmentType.id} className={styles.tableRow}>
                <td>{appointmentType.id}</td>
                <td className={styles.name}>{appointmentType.name}</td>
                <td>
                  <span
                    className={`${styles.statusBadge} ${appointmentType.isActive ? styles.statusActive : styles.statusInactive}`}
                  >
                    {appointmentType.isActive ? 'Hoạt động' : 'Tạm dừng'}
                  </span>
                </td>
                <td>{formatDate(appointmentType.createdAt)}</td>
                <td>
