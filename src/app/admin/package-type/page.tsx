...
import { PackageTypeModal } from './components/PackageTypeModal';
import { ImportPackageTypeModal } from './components/ImportPackageTypeModal';
import { ConfirmModal } from '@/components/ui/modal/ConfirmModal';
import styles from './package-type.module.css';
...
const sortItems = (items: PackageType[], key: SortKey) => {
  const arr = [...items];
  switch (key) {
    case 'id-asc':    return arr.sort((a, b) => a.id - b.id);
    case 'id-desc':   return arr.sort((a, b) => b.id - a.id);
    case 'name-asc':  return arr.sort((a, b) => a.typeName.localeCompare(b.typeName));
    case 'name-desc': return arr.sort((a, b) => b.typeName.localeCompare(a.typeName));
    default: return arr;
  }
};

export default function AdminPackageTypePage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<PackageType | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(getErrorMessage(err, 'Không thể tải danh sách loại gói dịch vụ'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredItems = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((item) => item.typeName.toLowerCase().includes(q));
    }
    if (statusFilter !== 'all') {
  ...
  const handleOpenEdit   = (item: PackageType) => { setEditingItem(item); setIsModalOpen(true); };
  const handleModalClose = (open: boolean) => { setIsModalOpen(open); if (!open) setEditingItem(null); };

  const handleDeleteTrigger = (item: PackageType) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await packageTypeService.deletePackageType(itemToDelete.id);
      await fetchData();
      toast({ title: 'Xóa loại gói dịch vụ thành công', variant: 'success' });
    } catch (err) {
      toast({ title: getErrorMessage(err, 'Xóa loại gói dịch vụ thất bại'), variant: 'error' });
    } finally {
      setDeletingId(null);
  ...
  const handleRestore = async (item: PackageType) => {
    try {
      setDeletingId(item.id);
      await packageTypeService.restorePackageType(item.id);
      await fetchData();
      toast({ title: 'Khôi phục loại gói dịch vụ thành công', variant: 'success' });
    } catch (err) {
      toast({ title: getErrorMessage(err, 'Khôi phục loại gói dịch vụ thất bại'), variant: 'error' });
    } finally {
      setDeletingId(null);
    }
  };

  const handleToggleStatus = async (item: PackageType, newStatus: boolean) => {
    ...
    try {
      ...
      if (newStatus) {
        await packageTypeService.restorePackageType(item.id);
        toast({ title: 'Khôi phục loại gói thành công', variant: 'success' });
      } else {
        await packageTypeService.deletePackageType(item.id);
        toast({ title: 'Tạm dừng loại gói thành công', variant: 'success' });
      }
      await fetchData();
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await packageTypeService.exportPackageTypes();
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const blob = await packageTypeService.downloadTemplatePackageTypes();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Mau_nhap_loai_goi_dich_vu.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    ...
    <div className={styles.controls}>
      <div className={styles.controlsLeft}>
        <div className={styles.searchWrapper}>
          ...
          <input type="text" placeholder="Tìm kiếm loại gói dịch vụ..." className={styles.searchInput} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-0 bg-white">
      <AdminPageLayout controlPanel={controlPanel} pagination={pagination}>
            ...
            <tbody>
              {error ? (
                ...
                <tr><td colSpan={4} className={styles.emptyState}>
                  {searchQuery ? 'Không tìm thấy kết quả phù hợp.' : 'Chưa có loại gói dịch vụ nào.'}
                </td></tr>
              ) : (
                paginatedItems.map((item, index) => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td className={styles.nameCell}>{item.typeName}</td>
                      <td>
        ...
        <PackageTypeModal open={isModalOpen} onOpenChange={handleModalClose} item={editingItem} onSuccess={fetchData} />
        <ConfirmModal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} onConfirm={handleConfirmDelete} title="Xác nhận xóa loại gói dịch vụ" message={`Bạn có chắc chắn muốn xóa loại gói dịch vụ "${itemToDelete?.typeName}"? Hành động này không thể hoàn tác.`} confirmLabel="Xóa ngay" cancelLabel="Suy nghĩ lại" variant="danger" />
        <ImportPackageTypeModal open={isImportModalOpen} onOpenChange={setIsImportModalOpen} onSuccess={fetchData} />
      </AdminPageLayout>
    </div>
  );
}
