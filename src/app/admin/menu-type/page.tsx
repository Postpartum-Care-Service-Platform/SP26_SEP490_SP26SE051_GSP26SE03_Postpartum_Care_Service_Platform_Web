...
const sortItems = (items: MenuType[], key: SortKey) => {
  const arr = [...items];
  switch (key) {
...
export default function AdminMenuTypePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(getErrorMessage(err, 'Không thể tải danh sách loại thực đơn'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleRestore = async (item: MenuType) => {
    try {
      setDeletingId(item.id);
      await menuTypeService.restoreMenuType(item.id);
      await fetchData();
      toast({ title: 'Khôi phục loại thực đơn thành công', variant: 'success' });
    } catch (err) {
      toast({ title: getErrorMessage(err, 'Khôi phục loại thực đơn thất bại'), variant: 'error' });
    } finally {
      setDeletingId(null);
    }
  };

  const handleToggleStatus = async (item: MenuType, newStatus: boolean) => {
    ...
    try {
      ...
      await menuTypeService.updateMenuType(item.id, { isActive: newStatus });
    ...
    <div className={styles.controls}>
      <div className={styles.controlsLeft}>
        <div className={styles.searchWrapper}>
          ...
          <input type="text" placeholder="Tìm kiếm loại thực đơn..." className={styles.searchInput} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-0 bg-white">
      <AdminPageLayout controlPanel={controlPanel} pagination={pagination}>
            ...
            <tbody>
              {error ? (
                ...
                <tr><td colSpan={4} className={styles.emptyState}>
                  {searchQuery ? 'Không tìm thấy kết quả phù hợp.' : 'Chưa có loại thực đơn nào.'}
                </td></tr>
              ) : (
        ...
        <MenuTypeModal open={isModalOpen} onOpenChange={handleModalClose} item={editingItem} onSuccess={fetchData} />
        <ConfirmModal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} onConfirm={handleConfirmDelete} title="Xác nhận xóa loại thực đơn" message={`Bạn có chắc chắn muốn xóa loại thực đơn "${itemToDelete?.name}"? Hành động này không thể hoàn tác.`} confirmLabel="Xóa ngay" cancelLabel="Suy nghĩ lại" variant="danger" />
        <ImportMenuTypeModal open={isImportModalOpen} onOpenChange={setIsImportModalOpen} onSuccess={fetchData} />
      </AdminPageLayout>
    </div>
  );
}
