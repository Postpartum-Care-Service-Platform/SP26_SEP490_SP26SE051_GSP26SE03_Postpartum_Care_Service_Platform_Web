...
import styles from './menu-type-table.module.css';
...
export function MenuTypeTable({ menuTypes, onEdit, onDelete, deletingId, pagination }: Props) {
  return (
        ...
        <tbody>
          {menuTypes.length === 0 ? (
            ...
            menuTypes.map((menuType) => (
              <tr key={menuType.id} className={styles.tableRow}>
                <td>{menuType.id}</td>
                <td className={styles.name}>{menuType.name}</td>
                <td>
                  <span
                    className={`${styles.statusBadge} ${menuType.isActive ? styles.statusActive : styles.statusInactive}`}
                  >
                    {menuType.isActive ? 'Hoạt động' : 'Tạm dừng'}
                  </span>
                </td>
                <td>{formatDate(menuType.createdAt)}</td>
                <td>{formatDate(menuType.updatedAt)}</td>
                <td>
                  <div className={styles.actions}>
                    {onEdit && (
                      <Button
                        ...
                        aria-label={`Chỉnh sửa ${menuType.name}`}
                      >
                    ...
                    {onDelete && (
                      <Button
                        ...
                        onClick={() => onDelete(menuType)}
                        disabled={deletingId === menuType.id}
                        aria-label={`Xóa ${menuType.name}`}
                      >
